...
def main():
    ...
    try:
        ...
        xml_files_dir = Path("resources/xml_files")
        to_correct_dir = Path("resources/to_correct")

        # Vérifier qu'il y a des fichiers .xml dans xml_files
        xml_files_exist = False
        if xml_files_dir.exists():
            xml_files_exist = any(f.suffix.lower() == '.xml' for f in xml_files_dir.iterdir() if f.is_file())

        # Vérifier qu'il y a des fichiers .txt dans to_correct
        to_correct_exist = False
        if to_correct_dir.exists():
            to_correct_exist = any(f.suffix.lower() == '.txt' for f in to_correct_dir.iterdir() if f.is_file())

        if not xml_files_exist or not to_correct_exist:
            print()
            print_warning("=" * 70)
            ...
            if not xml_files_exist:
                print_info(f"  1. Placez vos fichiers XML (.xml) dans :")
                print(f"     {os.path.abspath('resources/xml_files')}")
                print()
            if not to_correct_exist:
                print_info(f"  2. Placez vos fichiers texte (.txt) à corriger dans :")
                print(f"     {os.path.abspath('resources/to_correct')}")
                print()
